...
def print_header():
    ...
    print(f"  {config.NUM_TEAMS}-Team H2H Categories | Auction ${config.AUCTION_BUDGET}")
    print("=" * 60)
    print()
...
def print_league_settings():
    ...
    print(f"  Budget: ${config.AUCTION_BUDGET} per team (${config.TOTAL_LEAGUE_BUDGET} total)")
    ...
    print(f"  Budget split: {config.HITTER_BUDGET_PCT:.0%} hitters / {config.PITCHER_BUDGET_PCT:.0%} pitchers")
    print()
...
def print_positional_scarcity(hitters: pd.DataFrame):
    ...
    for pos in positions:
        ...
        pos_players = hitters[hitters["positions"].str.contains(pos, case=False, na=False)]
        ...
        if values:
            ...
            print(f"  {pos:<3}: ${top_val:>3} (top: {names[0]:<20}) | drop to #{len(values)}: ${drop}")
...
def save_outputs(
    ...
    h_cols = ["name", "positions", "dollar_value", "tier",
              "R", "HR", "RBI", "SB", "OBP", "PA",
              "z_R", "z_HR", "z_RBI", "z_SB", "z_OBP", "z_total"]
    h_cols = [c for c in h_cols if c in hitters.columns]
    hitters.sort_values("dollar_value", ascending=False)[h_cols].to_csv(h_path, index=False)
    print(f"  Saved: {h_path}")

    # Detailed pitcher valuations
    p_path = os.path.join(output_dir, "pitchers_valued.csv")
    p_cols = ["name", "positions", "dollar_value", "tier",
              "K", "W", "SV", "ERA", "WHIP", "IP",
              "z_K", "z_W", "z_SV", "z_ERA", "z_WHIP", "z_total"]
    p_cols = [c for c in p_cols if c in pitchers.columns]
    pitchers.sort_values("dollar_value", ascending=False)[p_cols].to_csv(p_path, index=False)
    print(f"  Saved: {p_path}")


def generate_summary(cheatsheet: pd.DataFrame, hitters: pd.DataFrame, pitchers: pd.DataFrame) -> str:
    ...
    for rank, row in cheatsheet.head(10).iterrows():
        lines.append(f"  {rank:>2}. {row['name']:<25} ${row['dollar_value']:>3}  ({row['player_type']})")
    lines.append("")
...
def main():
    ...
    parser.add_argument("--hitters", required=True, help="Path to hitter projections CSV")
    parser.add_argument("--pitchers", required=True, help="Path to pitcher projections CSV")
    parser.add_argument("--output", default="output", help="Output directory (default: output)")
    parser.add_argument("--min-pa", type=int, default=None, help="Override minimum PA threshold")
    parser.add_argument("--min-ip", type=int, default=None, help="Override minimum IP threshold")
    parser.add_argument("--hitter-budget-pct", type=float, default=None, help="Override hitter budget % (0-1)")
